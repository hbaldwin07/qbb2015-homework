...
plt.figure()
v = venn3(subsets= (count2, count3, count4, count5, count6, count7, count8), set_labels = ("CTCF", "BEAF", "Su(HW)"))
c = venn3_circles(subsets = (count2, count3, count4, count5, count6, count7, count8))
plt.savefig("d4lunchvenn.png")

print "Count(C-len): %d, Count(B-len): %d, Count4(S-len): %d, Count(C-B): %d, Count(C-S): %d, Count(B-S): %d,  Count(C-S-B): %d" % (count2, count3, count4, count5, count6, count7, count8)
